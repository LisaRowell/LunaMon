...
void WiFiManager::begin() {
  ...
  // The WiFiNINA library will sit and spin waiting for a begin method WiFi
  // connection attempt to succeed. While this is fine for toy sketches, and for
  // uses where a connection is established before anything else happens, it's not
  // appropriate for when we lose our connection while devices are streaming data
  // at us. Set a "timeout" that's small so that the method will return quickly.
  // If it didn't connect in that short time (likely), we'll periodically check
  // the status looking for a successful connection while continueing to keep
...
void WiFiManager::initiateConnection() {
  ...
  if (WiFi.begin(wifiSSID, wifiPassword) == WL_CONNECTED) {
    ...
    // The WiFi connection has not yet succeeded, but this isn't that
    // surprising since we gave it a short timeout to avoid locking
    // out other processing, like collecting sensor data. Set a retry
    // timer and check back on the status when we next get called.
...
void WiFiManager::registerForNotifications(WiFiManagerClient *client) {
  ...
  // If, by chance, we're already connected to the network, make sure that the new client
  // is informed.
  if (connected()) {
