...
void DataModelLeaf::unsubscribe(DataModelSubscriber *subscriber) {
  ...
  // MQTT protocol wide, tt's an acceptable occurance for a broker to receive an
  // unsubscribe for a topic that it doesn't have an active subcription for. We
  // didn't find the subscriber on the topic, but just calmly return.
}